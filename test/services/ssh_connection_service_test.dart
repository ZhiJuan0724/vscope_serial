...
void main() {
  ...
  test('SSH 认证连接后自动启动 PTY，仍可独立停止和重新启动', () async {
    ...
    expect(adapter.lastColumns, 80);
    expect(adapter.lastRows, 24);
  ...
  test('远端关闭 PTY 时报告原因并收敛整个 SSH 连接', () async {
    final adapter = _FakeSshAdapter();
    final service = SshConnectionService(
      connectionOwners: owners,
      connector: (config, secrets, trustedHost) async => adapter,
    ...
    adapter.closeShellFromRemote();
    final error = await errorFuture;
    await Future<void>.delayed(Duration.zero);

    expect('$error', contains('SSH 终端已由远端关闭'));
    expect(service.isConnected, isFalse);
    expect(owners.owner, ConnectionOwner.none);
  });

  test('逐键产生的并发发送在服务层严格保持串行', () async {
    final adapter = _FakeSshAdapter(
    ...
    await Future.wait([
      service.write(Uint8List.fromList([1])),
      service.write(Uint8List.fromList([2])),
      service.write(Uint8List.fromList([3])),
    ]);

    expect(adapter.writes, const [
      [1],
      [2],
      [3],
    ]);
    expect(adapter.maxConcurrentWrites, 1);
  });
  ...
  test('SSH 配置不序列化密码并规范化端口', () {
    ...
    expect(config.savePassword, isTrue);
    expect(config.toJson().containsKey('password'), isFalse);
    expect(config.toJson()['savePassword'], isTrue);
  });
}

class _FakeSshAdapter implements SshClientAdapter {
  _FakeSshAdapter({this.writeDelay = Duration.zero});
  ...
  final Completer<void> _shellDone = Completer<void>();
  ...
  @override
  Future<void> get shellDone => _shellDone.future;
  @override
  ...
  void closeShellFromRemote() {
    shellOpen = false;
    if (!_shellDone.isCompleted) _shellDone.complete();
  }
}
