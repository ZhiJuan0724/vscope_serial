...
void main() {
  test(
    'FileLogOutput creates per-process log files instead of latest.log',
    () async {
  ...
  test(
    'AppLogger only persists debug records when diagnostic mode is on',
    () async {
      ...
      logger.debug('diagnostic-disabled-marker', category: 'TEST');
      logger.setDiagnosticEnabled(true);
      logger.debug('diagnostic-enabled-marker', category: 'TEST');
      await logger.disposeLogger();

      final contents = await File(logPath).readAsString();
      expect(contents, isNot(contains('diagnostic-disabled-marker')));
      expect(contents, contains('diagnostic-enabled-marker'));
    },
  );
}
