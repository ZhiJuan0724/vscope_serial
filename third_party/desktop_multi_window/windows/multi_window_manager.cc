...
MultiWindowManager *MultiWindowManager::Instance() {
  // Flutter destroys the engine and its binary messenger before process-wide
  // static destructors run. Keep the shared owner alive until process exit so
  // WindowChannel is not destructed after its messenger has become invalid.
  static auto *manager =
      new std::shared_ptr<MultiWindowManager>(std::make_shared<MultiWindowManager>());
  return manager->get();
}
