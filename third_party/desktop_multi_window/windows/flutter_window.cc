...
// The Flutter runner embeds its application icon as IDI_APP_ICON (101).
// Multi-window classes are registered by this plugin, so they must load the
// executable resource explicitly instead of using the system IDI_APPLICATION
// identifier with the executable module handle.
...
void RegisterWindowClass(WNDPROC wnd_proc) {
  if (class_registered_ == 0) {
    ...
    window_class.hIcon = static_cast<HICON>(LoadImage(
        window_class.hInstance, MAKEINTRESOURCE(kAppIconResourceId),
...
LRESULT FlutterWindow::MessageHandler(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
  ...
  switch (message) {
    case WM_GETMINMAXINFO: {
      ...
      min_max_info->ptMinTrackSize.x = Scale(720, scale_factor_);
      min_max_info->ptMinTrackSize.y = Scale(480, scale_factor_);
      return 0;
    }
    case WM_FONTCHANGE: {
