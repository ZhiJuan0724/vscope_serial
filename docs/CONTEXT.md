...
- FireWater、固定帧协议、Zobow、JustFloat 四类解析协议。
- 绘图协议拆分为接收协议和发送协议。内置发送协议为“无”和 `r协议`；Zobow 接收协议固定使用内置二进制初始化帧。
- 多通道颜色、名称、显示、偏置、缩放、线宽和点半径配置。
- X-X、Y-Y、统计范围、观察线、跟随光标和通道偏置交互。
- 大数据绘图 LOD 索引，支持 1M~40M 可见窗口上限。
- 大文件导入异步执行，并通过进度弹窗提示读取、解析和索引进度。
- 旧版虚拟示波器 DAT 导入固定解析 4 通道小端 `int16`，跳过前 50,000 点预留区，并保留文件中的通道地址，不覆盖用户别名。
...
- 打开串口连接弹窗和开始绘图前会刷新串口列表，并通过原生串口 API 校验现有句柄；外部断线后若端口仍存在则自动重连一次，否则更新为断开状态。
- 原生串口打开在后台 isolate 执行，避免 Windows `CreateFile` 阻塞 UI；连接失败通过浮动 SnackBar 提示用户检查端口占用或设备状态。
...
- `sendProtocolType` 默认 `none`；`rChannelAddresses` 保存 16 个 r 地址文本槽位并保留十进制或 `0x` 输入形式；`rProfileId` 独立记录 r 配置选择。
- 已预留收发 Lua 自定义协议 ID 字段，本轮不扫描或执行 Lua 脚本。
...
- `SendProtocolConfig` 支持 `none`、`rProtocol` 和 Zobow 强制使用的 `zobowBuiltIn`，并预留 Lua 来源与自定义协议 ID。
...
- FixedFrame：可配置可选帧头、帧尾和固定通道数；数据类型可统一配置或在通道列表中逐通道设置，支持 CRC-8/16/32 多项式、CRC 位于帧尾前后以及大小端字节序。
- Zobow：无帧头滑动窗口，CRC16/MODBUS 校验；4 通道 10 字节，8 通道 18 字节。
- Zobow 解析器弹窗只选择 4/8 通道；地址和 `uint16/int16` 类型在通道面板中维护。停止绘图后修改类型会分批重新解释原始帧并重建 LOD，大数据量时显示进度弹窗。
- JustFloat：VOFA 小端 `float32` 数组 + `00 00 80 7F` 帧尾；通道数为 0 时按帧长度自动识别，最多 16 通道。

### 约束

- FireWater 只面向 ASCII 数字文本。
- FixedFrame 校验兼容 SUM8/SUM16，并支持 CRC-8/16/32。
- FixedFrame 通道数固定为 `1~16`，不支持自动识别；帧头和帧尾不能同时全部为 `0`。使用 r 协议时发送固定通道数量的地址，显式地址 `0` 会正常发送。
...
- 非 Zobow 接收协议可选择 `r协议`，启动前按连续非零地址构造 `r 地址1 地址2 ...\n`；固定通道协议要求地址数量足够，自动通道协议使用连续前缀。
- FireWater 和 JustFloat 为自动识别通道时，尚未开始绘图会展示全部 16 个通道槽位，便于预填多通道 r 地址；接收解析仍按实际数据自动识别。
- 随机源没有串口时若选择 `r协议`，自动持久化切回“无”并继续启动随机绘图。
...
- Zobow 配置文件支持选择、新建、编辑、删除、JSON 预设导入、从 C 文件/粘贴代码识别 `ChxValueTable` 函数内最长 `switch` 的预设和列表/平铺视图记忆；C 导入可选择忽略注释并全部使用变量名。
- r 配置文件与 Zobow 配置文件共用 JSON 结构并通过 `protocolType` 分组，r 配置弹窗仅提供 JSON 导入。旧 JSON 缺少 `protocolType` 时仍按 Zobow 处理。
- 观察线使用顶部刻度栏手柄，右键删除。
- X-X/Y-Y 测量手柄位于刻度栏，不侵入绘图区。
- 高级设置包含网格、刷新率输入、字体大小、吸附点高亮和绘图窗口上限。
- 抗锯齿固定开启，不再提供高级设置开关。
- 操作结果和错误统一使用浮动 SnackBar 临时提示；底部状态栏只保留串口连接和随机源状态。
