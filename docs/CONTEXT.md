...
- Windows Runner 在 Flutter 引擎初始化前安装未处理异常回调，默认将 Dart 无法捕获的原生崩溃写入 `<exe_dir>/crash_dumps/`，内容包括小型 minidump 和格式化 JSON 元数据；下一次启动只提示尚未查看的记录并最多保留最近 10 份。高级设置通过 `<exe_dir>/settings/crash_dump.disabled` 与下一次原生启动同步禁用状态，崩溃现场不得解析 `settings.json`。Debug 构建的诊断设置提供带二次确认的真实原生崩溃测试按钮及 DLL 入口；Release 界面必须隐藏该按钮，Release DLL 也不得导出测试入口。发布包继续排除 PDB，CI 和本地发布脚本必须为每个发布版本单独归档对应符号文件。`tools/analyze_crash_dump.ps1` 兼容 Windows PowerShell 5.1 和 PowerShell 7，可在独立转储目录中检查文件及 PDB 匹配状态并生成中文 Markdown 报告。
