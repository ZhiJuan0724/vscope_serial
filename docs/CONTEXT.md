...
- 大范围质量分为性能、均衡和质量优先；低密度直接绘制原始点，高密度按物理像素列生成真实时序的 first/min/max/last（M4），固定 LOD 桶只加速候选查询，不直接决定最终几何。
- 串口绘图可在 Canvas 与 Windows D3D11 间手动切换；两者必须消费同一份视口 M4 几何，D3D11 只替换曲线数据层并通过 DXGI 共享纹理合成，初始化或提交失败时保留历史并自动回退 Canvas。
- 性能档最多合并相邻两个物理像素列，均衡和质量逐列聚合；所有档位必须保持采样顺序、峰谷、阶跃、脉冲和孤立尖峰，不得以破坏拓扑换取帧率。
- 静态历史拖动使用有界 X/Y 预取纹理，只移动数据层；数据、通道、DPR、缩放或预取越界时回退 Canvas 并重建。纹理限制为 8192 单边和约 32M 像素，失败时保留历史并继续使用 Canvas。
- 坐标轴与网格在交互时最多 30 FPS；TypedData 缓冲只扩容不缩容。D3D11当前仅作为用户手动选择的Windows后端，完成更广泛硬件验证前不替代默认Canvas。
